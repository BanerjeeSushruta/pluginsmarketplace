...
The PDF generator now performs a post-merge TOC navigation pass. This fixes cases where the TOC appeared formatted but was not navigable after merging cover, body, and closing pages.

The generator now:

- Creates explicit internal `LINK_GOTO` annotations using PyMuPDF.
- Links TOC entries on page 2 to matching section heading pages.
- Returns `toc_navigation_explicit_links_created` in the generation log.
- Uses the strengthened `validate_toc_links.py` to confirm links target valid internal pages.
