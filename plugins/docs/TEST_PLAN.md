...
## Test Case 11: Validate Explicit Navigable TOC Links

Command:

```bash
python scripts/validate_toc_links.py out/sample_sre_assessment_toc_fixed.pdf   --summary out/toc_validation.json
```

Expected result:

- Status is success.
- TOC page contains internal GoTo link annotations.
- Every valid TOC link targets an existing page.
- TOC links navigate from the TOC page to generated section pages after page merge.
