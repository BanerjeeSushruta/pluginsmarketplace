...
## 18. Explicit Navigable TOC Link Enforcement

The skill must not rely only on ReportLab inline link annotations because link annotations can become unreliable after the cover page, body pages, and closing page are merged.

```yaml
explicit_toc_navigation:
  enabled: true
  post_merge_link_injection: true
  link_engine: PyMuPDF
  toc_page_number: 2
  link_type: internal_goto
  validate_target_page_exists: true
  fail_closed_if_no_links_created: true
```

Generation rule:

1. Generate the formatted TOC page.
2. Merge the PNG first page, free-flowing body pages, and PNG last page.
3. Search the finalized PDF for TOC entry text on the TOC page.
4. Search the finalized PDF for section heading text on body pages.
5. Add explicit internal `LINK_GOTO` annotations from each TOC entry to the matching section page.
6. Validate that the links are internal GoTo links and target valid pages.

Validation command:

```bash
python scripts/validate_toc_links.py out/sample_sre_assessment_toc_fixed.pdf \
  --summary out/toc_validation.json
```
