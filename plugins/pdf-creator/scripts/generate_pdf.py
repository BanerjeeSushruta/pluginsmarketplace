...
def add_toc_navigation(pdf_path, data, toc_page_index=1):
    """Add explicit GoTo links from the TOC page to section pages in final PDF.

    ReportLab link annotations can be lost or become unreliable after merging
    externally generated cover/closing pages. This post-merge pass searches the
    finalized PDF and adds explicit PyMuPDF LINK_GOTO annotations.
    """
    ...
    for idx, section in enumerate(data.get("sections", []), start=1):
        ...
        if not title:
            continue
        target_page = None
        # Skip cover and TOC page. Middle pages start after the TOC.
        for page_no in range(toc_page_index + 1, doc.page_count - 1):
    ...
    return {"toc_links_created": created, "toc_link_warnings": missing}
