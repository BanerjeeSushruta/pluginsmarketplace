...
def build_toc_table(data, styles, content_width):
    ...
    toc_rows = [[Paragraph("Section", styles["TableHeader"]), Paragraph("Page", styles["TableHeader"])]]
    for idx, section in enumerate(data.get("sections", []), start=1):
        ...
        toc_rows.append([Paragraph(link, styles["TableCell"]), Paragraph("", styles["TableCell"])])
...
def add_toc_navigation(pdf_path, data, toc_page_index=1):
    """Add explicit GoTo links and correct page numbers to the TOC page.

    Important: section titles also exist on the TOC page, so target-page search
    must skip all TOC pages. Otherwise every target resolves to the TOC page or
    the first content page and displayed page numbers become incorrect.
    """
    doc = fitz.open(pdf_path)
    if doc.page_count <= toc_page_index:
        raise RuntimeError("TOC page is missing from generated PDF.")

    layout_cfg = data.get("layout", {})
    toc_page_count = int(layout_cfg.get("toc_page_count", 1) or 1)
    search_start_page = toc_page_index + toc_page_count
    search_end_page = doc.page_count - 1  # Exclude closing/background page.
    if search_start_page >= search_end_page:
        raise RuntimeError("No body pages available after the TOC page for TOC target search.")

    toc_page = doc[toc_page_index]
    created = 0
    page_numbers_written = 0
    missing = []
    target_map = {}
    ...
    for idx, section in enumerate(data.get("sections", []), start=1):
        ...
        target_page = None
        # Search only body pages AFTER the TOC pages. Do not search the TOC page,
        # because the TOC itself contains every section title.
        for page_no in range(search_start_page, search_end_page):
            if doc[page_no].search_for(title):
                target_page = page_no
                break
        if target_page is None:
            missing.append(title)
            continue
        target_map[title] = target_page + 1  # 1-based visible PDF page number.

        rects = toc_page.search_for(title)
        if not rects:
            missing.append(f"TOC entry not found: {title}")
            continue

        # Use the first matching TOC title occurrence only. This prevents duplicate
        # link annotations when the same title appears more than once in TOC text.
        rect = rects[0]
        expanded = fitz.Rect(rect.x0 - 2, rect.y0 - 2, min(toc_page.rect.width - 20, rect.x1 + 220), rect.y1 + 2)
        ...
        # Write the final, post-merge page number into the TOC page's right column.
        page_num_text = str(target_page + 1)
        number_x = toc_page.rect.width - 92
        number_y = rect.y1 - 1
        toc_page.insert_text(
            ...
            fontsize=8.4,
            fontname="helv",
            color=(0.0, 0.227, 0.439),
        )
        page_numbers_written += 1

    if created == 0:
        raise RuntimeError("No navigable TOC links were created.")
    tmp_path = str(pdf_path) + ".toc.tmp"
    doc.save(tmp_path, garbage=4, deflate=True)
    doc.close()
    Path(tmp_path).replace(pdf_path)
    return {
        "toc_links_created": created,
        "toc_page_numbers_written": page_numbers_written,
        "toc_target_page_map": target_map,
        "toc_search_start_page": search_start_page + 1,
        "toc_link_warnings": missing,
    }
...
def main():
    ...
    toc_nav = add_toc_navigation(out_path, data)
    ...
    print(json.dumps({
        ...
        "toc_clickable": True,
        "toc_formatted": True,
        "toc_navigation_explicit_links_created": toc_nav["toc_links_created"],
        "toc_page_numbers_written": toc_nav["toc_page_numbers_written"],
        "toc_target_page_map": toc_nav["toc_target_page_map"],
        "toc_target_search_start_page": toc_nav["toc_search_start_page"],
        "toc_navigation_warnings": toc_nav["toc_link_warnings"],
